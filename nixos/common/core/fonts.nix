{ pkgs, lib, ... }:
{
  ...
  fonts = {
    ...
    packages = [
      ...
      # source-sans # 无衬线字体，不含汉字。字族名是 Source Sans Pro（旧版）和 Source Sans 3（新版），以及变体。
      # source-serif # 衬线字体，不含汉字。字族名应为 Source Serif Pro（旧版）或 Source Serif 4（新版），以及变体。
      # source-code-pro # 等宽编程字体，字族名为 Source Code Pro。
